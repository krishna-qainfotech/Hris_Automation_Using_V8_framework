...
public class YamlReader {
	...
	public static String getYamlValue(String token) {
		try {
			System.out.println("######Util.YamlReader.getYamlValue string token "+token);
	...
	private static String getValue(String token) throws FileNotFoundException {
		Reader doc = null;
		try {
			System.out.println("#####YamlReader.getValue yamlFilePath : "+yamlFilePath);
			doc = new FileReader(yamlFilePath);
		} catch (FileNotFoundException e) {
		...
		Map<String, Object> object = (Map<String, Object>) yaml.load(doc);
		System.out.println("#####YamlReader.getValue HashMap<string , object> : "+object.get(object));
